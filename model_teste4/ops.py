...
def weight_norm(w):
    ...
    g = tf.compat.v1.get_variable('wn_g', initializer=g_init)
...
def conv2d(inp, filters, kernel_size, strides=(1, 1), padding='reflect', dilation_rate=1, activation=None,
    ...
    _name = name if name_key is None else f'conv_{name_key}'

    with tf.compat.v1.variable_scope(_name):
        # Ajuste de padding refletivo, se necessário
        if padding == 'reflect':
        ...
        if bias_init_hook is not None:
            ...
            b = tf.compat.v1.get_variable(name='bias', shape=b_shape,
                                initializer=tf.constant_initializer(b_init), trainable=trainable)

        # Aplicação da SeparableConv2D com depth_multiplier = 1
        separable_conv = tf.keras.layers.SeparableConv2D(
            filters=filters,                   # Saídas finais
            kernel_size=_ks,                   # Tamanho do kernel
            strides=_sts,                      # Passo da convolução
            padding='same' if padding == 'same' else 'valid',
            dilation_rate=dilation_rate,       # Taxa de dilatação
            depth_multiplier=1,                # Apenas 1 filtro por canal
            use_bias=False,                    # Bias tratado separadamente
            trainable=trainable,
            name='separable_conv'
        )

        # Convolução separável + Bias
        out = separable_conv(x) + b

        # Aplicação da função de ativação, se definida
        if activation is not None:
            out = activation(out)

    return out

def xconv2d(inp, filters, kernel_size, strides=(1, 1), padding='reflect', dilation_rate=1, activation=None,
           trainable=True, name=None, name_key=None, b_init=0.0, kernel_init_hook=None, bias_init_hook=None):
    assert name is not None or name_key is not None
    assert padding in ['reflect', 'valid', 'same'], padding
    assert dilation_rate in [1, 2], dilation_rate
    ...
    with tf.compat.v1.variable_scope(_name):
        ...
        if kernel_init_hook is not None:
            ...
            w = tf.compat.v1.get_variable(shape=w_shape, initializer=he_normal(), trainable=trainable, name='kernel')
        if bias_init_hook is not None:
            b = bias_init_hook(b_shape)
        else:
            b = tf.compat.v1.get_variable(name='bias', shape=[filters], trainable=trainable,
                                initializer=tf.constant_initializer(b_init))

        # Use SeparableConv2D instead of tf.nn.conv2d
        separable_conv = tf.keras.layers.SeparableConv2D(filters=filters, 
                                                          kernel_size=_ks, 
                                                          strides=_sts, 
        ...
        out = separable_conv(x) + b  # Add bias after the separable convolution

        if activation is not None:
            out = activation(out)
    return out

def dconv2d(inp, filters, kernel_size, strides=(1, 1), padding='reflect', dilation_rate=1, activation=None,
            trainable=True, name=None, name_key=None, b_init=0.0, kernel_init_hook=None, bias_init_hook=None):
    assert name is not None or name_key is not None
    assert padding in ['reflect', 'valid', 'same'], padding
    assert dilation_rate in [1, 2], dilation_rate
    ...
    _ks = (kernel_size, kernel_size) if isinstance(kernel_size, int) else kernel_size
    _sts = (strides, strides) if isinstance(strides, int) else strides
    _name = name if name_key is None else 'dconv_{}'.format(name_key)
    ...
    with tf.compat.v1.variable_scope(_name):
        ...
        depthwise_conv = tf.keras.layers.DepthwiseConv2D(
            ...
            dilation_rate=dilation_rate,
            depth_multiplier=1,
            use_bias=False,  # No bias in depthwise
            trainable=trainable,
            name='depthwise_conv'
        )(x)

        # Bias initialization
        b_shape = [inp.get_shape().as_list()[3]]  # Same as the number of input channels
        if bias_init_hook is not None:
            b = bias_init_hook(b_shape)
        else:
            b = tf.compat.v1.get_variable(name='bias', shape=b_shape, trainable=trainable,
                                initializer=tf.constant_initializer(b_init))

        out = depthwise_conv + b  # Add bias after depthwise convolution
...
def sn_kernel_init_hook(shape):
    w = tf.compat.v1.get_variable(shape=shape, trainable=True, name='kernel', initializer=he_normal())
    return spectral_norm(w)


def wn_kernel_init_hook(shape):
    w = tf.compat.v1.get_variable(shape=shape, trainable=True, name='kernel',
...
def up_sample_resize(inp, kernel_size, oh, ow, filters=None, activation=None, name=None, name_key=None,
    ...
    filters = c if filters is None else filters
    x = tf.compat.v1.image.resize_nearest_neighbor(inp, (oh, ow), align_corners=True, name=name + '_resize')
...
def xstandard_res_unit(input_tensor, filters, kernel_size=3, strides=1, act=tf.nn.relu, training=True,
...
def standard_res_unit(input_tensor, filters, kernel_size=3, strides=1, act=tf.nn.relu, training=True,
    ...
    with tf.compat.v1.variable_scope(name):
        # Primeira Depthwise Convolution
        x = dconv2d(x, filters, kernel_size, strides=1, name_key='0',
                    kernel_init_hook=kernel_init_hook, bias_init_hook=bias_init_hook)
        
        x = norm(x, training=training, name_key='0')
        x = act(x) if act is not None else x

        # Segunda Depthwise Convolution
        x = dconv2d(x, filters, kernel_size, strides=strides, name_key='1',
                    kernel_init_hook=kernel_init_hook, bias_init_hook=bias_init_hook)

        in_c = input_tensor.get_shape().as_list()[-1]
        if in_c != filters:
            input_tensor = dconv2d(input_tensor, filters, 1, 1, name_key='btn',
                                    kernel_init_hook=kernel_init_hook, bias_init_hook=bias_init_hook)
        
        # Adiciona a entrada ao resultado da segunda convolução
        x = tf.add(x, input_tensor, name='res')

        # Pointwise Convolution
        x = tf.keras.layers.Conv2D(filters, kernel_size=1, padding='same',
                                   use_bias=False, trainable=True, name='pointwise_conv')(x)
