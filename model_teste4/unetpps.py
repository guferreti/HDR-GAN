...
class UnetppGeneratorS(BaseModel):
    ...
    def graph(self, image1, image2, image3, train=True, reuse=None, summary_feat=False, get_features=False,
        ...
        with tf.compat.v1.variable_scope(self.name, reuse=reuse):
            ...
            with tf.compat.v1.variable_scope('backbone_1' if not self.one_encoder else 'backbone_0'):
                features1, stage_ref = self.encoder(image2, filters)

            _reuse = True if self.one_encoder else None
            with tf.compat.v1.variable_scope('backbone_0', reuse=_reuse):
                features0, stage_00 = self.encoder(image1, filters)

            with tf.compat.v1.variable_scope('backbone_2' if not self.one_encoder else 'backbone_0', reuse=_reuse):
            ...
            if image1_ref is not None:
                with tf.compat.v1.variable_scope('backbone_0', reuse=True):
                    features0_ref, stage_00_ref = self.encoder(image1_ref, filters)
            if image3_ref is not None:
                with tf.compat.v1.variable_scope('backbone_2' if not self.one_encoder else 'backbone_0', reuse=True):
            ...
            if summary_feat:
                def unit_feat_shape(features):
                    ...
                    return [tf.compat.v1.image.resize_nearest_neighbor(i, shape0, True)
            ...
            with tf.compat.v1.variable_scope('decoders'):
...
def merge_net(inp1, inp_ref, inp2, training, act, norm, conv2d, name_key):
    ...
    with tf.compat.v1.variable_scope('merge_net_{}'.format(name_key)):
